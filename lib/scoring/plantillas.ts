...
import type { DatosReporte, Reporte, ResultadoPerfil, SeccionReporte } from './tipos';
...
type PlantillaFn = (d: DatosReporte) => Contenido;
...
// - El nombre de la usuaria ya aparece en el título del resultado — no repetirlo como saludo aquí.
...
function construirNotaCondicionPrevia(condiciones: string[], embarazoLactancia: string): string {
  ...
  return `Como nos compartiste que tienes **${items.join(', ')}**, te recomendamos aplicar este protocolo en conjunto con tu médico, para asegurarnos de que todo se maneje de forma segura y coordinada.`;
}

function construirNotaHibrido(perfilSecundario: string | null): string {
  if (!perfilSecundario) return '';
  return `También notamos un componente secundario relacionado con el patrón **${nombreAmigablePerfil(perfilSecundario)}** — vale la pena tenerlo presente mientras avanzas.`;
}

export function generarReporte(resultado: ResultadoPerfil): Reporte {
  const perfilBase = resultado.perfil.includes('+') ? resultado.perfil.split('+')[0] : resultado.perfil;
  const { porQue, queHacer } = PLANTILLAS[perfilBase](resultado.contexto);

  const notas: string[] = [];
  const notaHibrido = construirNotaHibrido(resultado.perfil_secundario);
  if (notaHibrido) notas.push(notaHibrido);
  const notaCondicion = construirNotaCondicionPrevia(resultado.condiciones_marcadas, resultado.embarazo_lactancia);
  if (notaCondicion) notas.push(notaCondicion);

  const secciones: SeccionReporte[] = [
    { titulo: 'Tu patrón principal', parrafos: porQue },
    { titulo: 'Qué te recomendamos', parrafos: queHacer },
  ];
  if (notas.length > 0) {
    secciones.push({ titulo: 'Notas adicionales', parrafos: notas });
  }

  return {
    texto: [...porQue, ...queHacer, ...notas].join('\n\n'),
    secciones,
  };
}
